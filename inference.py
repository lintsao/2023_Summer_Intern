...
all_data['head_b_r'] = torch.cat([torch.tensor(config.head_pitch).unsqueeze(0), torch.tensor(config.head_yaw).unsqueeze(0)], dim=0).unsqueeze(0)
all_data['gaze_b_r'] = torch.cat([torch.tensor(config.gaze_pitch).unsqueeze(0), torch.tensor(config.gaze_yaw).unsqueeze(0)], dim=0).unsqueeze(0)
...
with torch.no_grad():
    ...
    save_images(output_dict["output_1024"][0], os.path.join(config.redirect_path, f"hp_{config.head_pitch}_hy_{config.head_yaw}_gp_{config.gaze_pitch}_gy_{config.gaze_yaw}_1024.png"), fromTransformTensor=True)
    save_images(output_dict["output_256"][0], os.path.join(config.redirect_path, f"hp_{config.head_pitch}_hy_{config.head_yaw}_gp_{config.gaze_pitch}_gy_{config.gaze_yaw}_256.png"), fromTransformTensor=True)
