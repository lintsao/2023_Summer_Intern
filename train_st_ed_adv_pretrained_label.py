...
def execute_visualize(data):
    ...
    for i in tqdm(range(len(keys))):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        save_images(output_dict['image_a_inv'][i], os.path.join(path, 'input_inv_image.png') + '.png', fromTransformTensor=True)
        save_images(output_dict['image_b_inv'][i], os.path.join(path, 'target_inv_image.png'), fromTransformTensor=True)
